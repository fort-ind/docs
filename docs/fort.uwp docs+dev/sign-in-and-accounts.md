...
sidebar_position: 3
---
# sign-in & accounts (MiAuth deep dive)

fort.uwp doesn't have its own accounts - signing in just links the app to your existing **fort.social** account (fort.social runs Sharkey/Misskey). This page walks through exactly how that works, mostly based on `Services\MisskeyAuthService.vb` and `Services\ProfileService.vb`.

## why not an embedded browser?

The obvious way to do this in UWP is `WebAuthenticationBroker`, which pops up an embedded web view. That doesn't work here - Sharkey's frontend is a modern SPA and hangs indefinitely inside WAB's legacy embedded browser control. So instead, the app opens the consent page in your **actual default browser** and gets control back a different way: protocol activation.
...
3. You approve (or deny) the sign-in on fort.social. If approved, fort.social redirects the browser to the `callback` URL above - a custom `fortind://` link.
...
- If the background refresh fails - say, the token was revoked on fort.social's side - the app quietly keeps showing the last-known cached profile rather than kicking you out. You only actually get signed out by hitting Sign Out yourself (or "clear login info" in Settings), which calls `ProfileService.LogoutAsync` to remove the token from the vault and clear the cached profile.

## what fort.uwp can and can't do with your account

The app only ever requests the `read:account` permission via MiAuth - it can read your profile, not post or manage your account. `ProfilePage` is intentionally **read-only**: fort.social is the source of truth, so any actual profile editing happens over there, not in the app.
